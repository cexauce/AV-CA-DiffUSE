...
AVH  = os.path.join(BASE, "av_hubert")
if AVH not in sys.path:
    sys.path.insert(0, AVH)  # use insert, not append!
...
from sgmse.util.models.cogenav import CoGenAV
...
def get_mouthroi_audio_pair_load_raw_video(
    ...
    if frames is not None :
        if video_feature_type in ["resnet", "avhubert", "cogenav", "cogenav_after_transformer"]:    #$            
...
def load_visual_data_for_enhancement(vfile_path, video_feature_type, vfeat_processing_order):        
    ...
    if vfeat_processing_order in ["cut_extract"]:            
        video_size_dict = {"avhubert":88, "cogenav": 88, "cogenav_after_transformer": 88, "resnet":88, "raw_image":88, "flow_avse":112}
...
def build_cogenav_extractor(ckpt_path="./checkpoints/base_cogen.pt"):
    print(".... Loading CoGenAV Model")
    model = CoGenAV(cfg_file="./checkpoints/base.yaml", model_tensor=ckpt_path)
    model.cuda()
...
def build_extractor(video_feature_type):
    ...
    if video_feature_type == "resnet":
        ...
        avhubert_ckpt_path = "/srv/storage/talc3@storage4.nancy.grid5000.fr/multispeech/calcul/users/jayilo/pretrained_model/finetune-model.pt"
    ...
    elif video_feature_type in ["cogenav", "cogenav_after_transformer"]:
    
        feature_extractor = build_cogenav_extractor()
