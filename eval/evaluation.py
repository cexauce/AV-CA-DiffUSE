...
import json
import torch
from src.enhancement import UDiffSE, fUDiffSE, fUDiffSE_v2
...
def speech_enhance(params):
    ...
    if params["dataset"] == "WSJ0":
        ...
        with open(params["data_dir"], "r") as f:
            dataset = json.load(f)
    elif params["dataset"] in ["TCD-DEMAND","TCD-TIMIT", "TCD-TIMIT-small", "TCD-TIMIT-LowSNRs", "LRS3-NTCD"]:
        dataset = load_dict(params["data_dir"])
             
    print(params["visual_ablation"])
    ...
    elif params["algo_type"] == "fudiffse_v2":        
        enhance = fUDiffSE_v2(
            ckpt_path=params["ckpt_path"], num_E=params["num_E"], verbose=verbose, set_v_to_zero=params["set_v_to_zero"]
        )
    print("video feature type : ", enhance.video_feature_type)
    ...
    elif params["dataset"] in ["TCD-TIMIT","TCD-TIMIT-small","TCD-DEMAND", "LRS3-DEMAND", "LRS3-NTCD", "TCD-DEMAND-LowSNRs", "LRS3-DEMAND-LowSNRs", "LRS3-NTCD-LowSNRs"]:
        ...
        for ind_i, mix_info in enumerate(dataset):
            if (
                    ...
                    if enhance.vfeat_processing_order in ["cut_extract"]: 
                        ...
                        if params["dataset"] in ["TCD-DEMAND", "TCD-TIMIT", "TCD-TIMIT-small", "TCD-TIMIT-LowSNRs"]:
                            ...
                            if enhance.video_feature_type in  ["resnet", "avhubert", "raw_image", "cogenav", "cogenav_after_transformer"]: 
                                ...
                                video_path  = "/srv/storage/talc3@storage4.nancy.grid5000.fr/multispeech/calcul/users/cmboungo/cogenav2/embeddings/test_data_NTCD/{speaker_id}/{filename}_cogenav_feats_before_transformer.npy"                                            
                ...
                s_hat, _ = enhance.run(
                    ...
                    nbatch=params["nbatch"],
                    startstep=params["startstep"],
                    visual_ablation=params["visual_ablation"]
...
class Options:
    ...
    def _initial(self):
        ...
        self.parser.add_argument(
            ...
            type=str,
            default="udiffse",
            choices=["udiffse","fudiffse","fudiffse_v2"],
        ...
        self.parser.add_argument(
            "--set_v_to_zero", type=str, choices=("zero", "no"), default="no", 
        ...
        self.parser.add_argument(
            "--visual_ablation", type=str, default="no", 
            help="whether to set v to 0 or not. If yes, then the video feature v=0,. this is purely for experimentation purpose."
        )         
