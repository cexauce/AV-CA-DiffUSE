...
class UDiffSE:
	...
	def run(
		...
		if not self.audio_only:        
			if self.vfeat_processing_order in ["cut_extract"]:
				if self.video_feature_type in  ["resnet", "avhubert", "cogenav", "cogenav_after_transformer"]: 
...
class fUDiffSE_v2:
	def __init__(
		...
		print("##### finishing launching model ######")
		self.audio_only = self.model.audio_only
		self.video_feature_type = None
		if not self.audio_only :
		...
		if not self.audio_only:
			...
			if self.vfeat_processing_order in ["cut_extract"]:
				if self.video_feature_type in  ["cogenav_pre", "avhubert_pre"]: 
					v=np.load(vfile_path)
					v = torch.from_numpy(v).to("cuda").permute(1,0).unsqueeze(0).unsqueeze(0)
					v = v.repeat(self.nbatch, 1, 1, 1)
					self.visual_feature = v
				else:
					v,_=self.load_visual_data(vfile_path)   
		else:
			v = None          
	...
	def predictor_corrector(self, St, t, v, laststep, dt, visual_ablation="no"):
		print(visual_ablation)

		
		with torch.no_grad():
			# Corrector
			score = self.model.forward(St, t, v, visual_ablation)
			std = self.sde.marginal_prob(St, t)[1]
			step_size = (self.snr * std) ** 2
			z = torch.randn_like(St)
			St = (
	...
	def posterior_sampler(self, Wt, Ht, startstep=0, visual_ablation="no"):
		"""
		Posterior sampler algorithm that functions as the E-step for the EM process of UDiffSE.
		"""
		timesteps = self.scheduler.timesteps()
	...
	def run(
		...
		# fUDiffSE algorithm
		St, N0 = self.posterior_sampler(Wt, Ht, startstep=startstep, visual_ablation = visual_ablation)
