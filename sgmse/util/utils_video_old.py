...
"""def build_cogenav_extractor(ckpt_path, user_dir):
    print(".... Loading CoGenAV Model")
    model = CoGenAV(cfg_file="config/base.yaml", model_tensor=ckpt_path)
    model.cuda()
...
def build_extractor(video_feature_type):
    ...
    """elif video_feature_type == "cogenav":

        cogenav_ckpt_path = "/srv/storage/talc3@storage4.nancy.grid5000.fr/multispeech/calcul/users/cmboungo/cogenav2/base_cogen.pt"
        cogenav_user_dir = "./sgmse/util/av_hubert/avhubert"
        feature_extractor = build_cogenav_extractor(
            cogenav_ckpt_path, cogenav_user_dir
        )"""
