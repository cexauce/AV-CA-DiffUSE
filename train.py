...
if __name__ == "__main__":
    ...
    for parser_ in (base_parser, parser):
        ...
        parser_.add_argument(
            "--video_feature_type",
            type=str,
            choices=("avhubert","avhubert_pre","raw_image", "resnet", "flow_avse", "cogenav", "cogenav_after_transformer", "cogenav_pre"), #$
            default="resnet",
    ...
    if "pretrained" in temp_args.run_id:
        max_epochs = 400
    else:
        max_epochs = 200
    ...
    trainer = pl.Trainer.from_argparse_args(
        ...
        strategy=DDPStrategy(find_unused_parameters=True ), #not_checking_unused_parameters),
        logger=logger,
        log_every_n_steps=50,
        num_sanity_val_steps=0,
        limit_val_batches=0, # <--- this fully disables validation loop
        callbacks=callbacks,
        accelerator="gpu",
    ...
    # Train model
    last_checkpoint = Path(f"./logs/{temp_args.run_id}/last.ckpt")

    if last_checkpoint.exists():
        if "pretrained" in temp_args.run_id:
            last_checkpoint = Path(f"./logs/{temp_args.run_id}/epoch=199-last.ckpt")
        else:
            last_checkpoint = Path(f"./logs/{temp_args.run_id}/last.ckpt")
