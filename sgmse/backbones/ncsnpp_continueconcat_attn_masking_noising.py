...
from .ncsnpp_utils.utils import MultiModalAttention, mask_spectrogram, audio_mel
...
#from sgmse.util.utils_video_old import (
#    build_extractor
#)
ResnetBlockDDPM = layerspp.ResnetBlockDDPMpp
ResnetBlockBigGAN = layerspp.ResnetBlockBigGANpp
AudioResNetEmbedder = layerspp.AudioResNetEmbedder
AudioFront = layerspp.AudioFront
vCAFE_masking = layerspp.VCAFE_Masking
...
class NCSNpp_continueconcat_attn_masking_noising(nn.Module):
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        self.mlp_before_fusion = unused_kwargs.get("mlp_before_fusion", False)
        self.perceptron_before_fusion = unused_kwargs.get("perceptron_before_fusion", False)
        self.beta0 =  unused_kwargs.get('regularization_beta0', 0.0)
        print(self.beta0)
        self.contrastive_loss = unused_kwargs.get('contrastive_loss', "info_nce")
        ...
        if not self.audio_only and self.video_feature_type not in [ "avhubert_pre", "cogenav_pre"]: #$

            if self.vfeat_processing_order in ["cut_extract"]:
                if self.video_feature_type in ["avhubert", "resnet", "cogenav", "cogenav_after_transformer"]:  #$
                    ...
                    self.v_mlp_adapter = None
                    self.v_perceptron_adapter = None
                    self._use_contrastive_audio_embedder = (
                        self.video_feature_type in ["avhubert", "cogenav"] and self.beta0 > 0
                    )
                    if self._use_contrastive_audio_embedder:  # AV contrastive loss
                        if self.mlp_before_fusion:
                            print('2 layer MLP before Fusion')
                            self.v_proj_in_dim = unused_kwargs.get("v_proj_in_dim", 768)      # Dv
                            # If not explicitly set, keep video MLP output aligned with AV-HuBERT size.
                            self.v_proj_out_dim = unused_kwargs.get("v_proj_out_dim", 768)
                            self.v_proj_hidden_dim = unused_kwargs.get("v_proj_hidden_dim", 512)

                            self.v_mlp_adapter = torch.nn.Sequential(
                                torch.nn.Linear(self.v_proj_in_dim, self.v_proj_hidden_dim),
                                torch.nn.GELU(),
                                torch.nn.Linear(self.v_proj_hidden_dim, self.v_proj_out_dim),
                            )
                            layers.mlp_weights_init(self.v_mlp_adapter)

                        elif self.perceptron_before_fusion:
                            print('Simple perceptron before Fusion')
                            self.v_perceptron_adapter = torch.nn.Linear(768, 768)
                            layers.mlp_weights_init(self.v_perceptron_adapter)
            ...
            if (
                ...
                self.video_feature_type in ["resnet", "avhubert", "cogenav", "cogenav_after_transformer"] #$
                ...
                elif self.video_feature_type in ["avhubert","cogenav", "cogenav_after_transformer"]: #$
                    video_embed = 768
                    self.new_embed_dim_video = 256
                    self.adapt_dim_layer = torch.nn.Linear(
                        video_embed, self.new_embed_dim_video
        ...
        #if self.video_feature_type in ["cogenav", "avhubert_pre"]:
        if self.video_feature_type in ["cogenav_pre", "cogenav", "cogenav_after_transformer", "avhubert_pre"]:
                self.new_embed_dim_video = 768
        if getattr(self, "_use_contrastive_audio_embedder", False):
            # Match audio and video embedding dimensions in contrastive branch.
            embed_dim = self.new_embed_dim_video
            self.audio_processor = AudioResNetEmbedder(in_ch=2, emb_dim=embed_dim)
            print("Audio ResNet18 : outdim = ", embed_dim)
        if self.fusion == "vcafe":
            self.vCAFE_audio_resnet = AudioFront(in_ch=1, emb_dim=self.new_embed_dim_video)
            print("vCAFE Audio ResNet18 : outdim = ", self.new_embed_dim_video)
            self.vCAFE_masking = vCAFE_masking()
        ...
        if not self.audio_only:
            ...
            elif self.fusion in ["concat_attn_masking_light_early_late", "early", "vcafe"]:
                x_dim = 63 if self.fusion=="vcafe" else  256
                self.attention_module = MultiModalAttention(x_feature_dim=x_dim, v_feature_dim=self.new_embed_dim_video, num_heads=1)
                num_channels=3
                Concat_CrossAttn = functools.partial(
                    Concat_CrossAttnBlock_Light,
                    add_noise=False,
            ...
            if self.fusion in ["concat_attn_masking","concat_attn_masking_light", "concat_attn_masking_light_early_late", "concat_attn_noising","concat_attn","attn_gate","concat_attn_masking_learned","simple_attn", "vcafe"]:
        ...
        if not self.audio_only and self.fusion not in  ["early", "vcafe"]:
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument(
            ...
            choices=[               
                "simple_attn",
                "concat_attn",
                "concat_attn_masking_light_early_late",
                "early",
                "vcafe",
                "concat_attn_noising",
                "concat_attn_masking",
        ...
        parser.add_argument(
            "--mlp_before_fusion",
            action="store_true",
            help="whether to add an MLP before fusion",
        )
        parser.add_argument(
            "--perceptron_before_fusion",
            action="store_true",
            help="whether to add a Residual MLP before fusion",
        )
    ...
    def forward(self, x, time_cond, v, grad=False, visual_ablation="no"):
        #print(visual_ablation)
    
        if not self.audio_only and self.fusion in ["concat_attn_masking_light_early_late", "early"]:
            # This is when the input is video data
            # x.shape: torch.Size([2, 256, 88, 88])
            
            v = v.to(dtype=x.abs().dtype)
            #print("v : ", v.shape)
            print("x before attention", x.abs().permute(0,1,3,2).shape)
            x_av = self.attention_module(x.abs().permute(0,1,3,2), v).permute(0,2,1)

            x_av = x_av.unsqueeze(1)

            x = torch.cat((x[:,[0],:,:].real, x[:,[0],:,:].imag, x_av[:,[0],:,:]), dim=1)
            
            # x.shape: torch.Size([2, 1, 256, 256])
        elif not self.audio_only and self.video_feature_type =="avhubert" and self.fusion =="vcafe" :
            # This is when the input is video data
            # x.shape: torch.Size([2, 256, 88, 88])
            
            v = v.to(dtype=x.abs().dtype)
            # AVHUBERT  Visual embedding
            """Before extraction, v :  torch.Size([4, 1, 131, 88, 88])
            after extraction, v :  torch.Size([4, 131, 768])"""

            # Normalization
            v = (v / 255 - 0.421) / 0.165
            print("Before extraction, v : ", v.shape)
            with torch.no_grad():
                
                f_v, _ = self.feature_extractor.extract_finetune(
                    source={"video": v, "audio": None},
                    padding_mask=None,
                    output_layer=None,
                )

            print("After extraction, v : ", f_v.shape)

            f_v = f_v.contiguous().unsqueeze(1)# f_v.transpose(-2, -1).contiguous()  # (b,512, t) or #(b,768, t)
            print("f_v : ", f_v.shape)

            #x = torch.squeeze(x)
            print("x shape : ", x.shape)
            f_a = self.vCAFE_audio_resnet(x.abs()).permute(0, 2, 1) # [B, T=63, D=768]
            print("f_a shape : ", f_a.shape)
            
            context = self.attention_module(f_a.unsqueeze(1), f_v).permute(0,2,1)

            context = context.unsqueeze(1)
            print("context shape : ", context.shape)  # [B, 1, T=63, D=768]

            x_av = self.vCAFE_masking(context) 

            print("x av embedding : ", x_av.shape)  # [2, 768, 63]
            x_av = x_av.permute(0, 2, 1).unsqueeze(1)
            x = torch.cat((x[:,[0],:,:].real, x[:,[0],:,:].imag, x_av[:,[0],:,:]), dim=1)
        else:
        ...
        if not self.audio_only and self.video_feature_type not in ["cogenav_pre", "avhubert_pre"]: #$ cogenav
            """Before extraction, v :  torch.Size([4, 1, 131, 88, 88])
            after extraction, v :  torch.Size([4, 131, 768])"""
            if visual_ablation=="yes":
                print("ablation")
                # B = batch size, T = number of frames, D = embedding dim 768
                B = v.shape[0]
                T = v.shape[2] if v is not None else 131  # default fallback
                device = x.device
                #v = torch.zeros(B, T, 768, device=device, dtype=x.dtype)
                v = torch.randn(B, T, 768, device=device, dtype=x.dtype)
                #print("random v : ", v)
            else :
                # Normalization
                v = (v / 255 - 0.421) / 0.165
                ...
                if self.vfeat_processing_order in ["cut_extract"]:
                    ...
                    if self.video_feature_type == "avhubert": 
                        ...
                        with torch.no_grad():
                            ...
                            #print("after extraction, v : ", v.shape)
                        if self.mlp_before_fusion:
                            #print("Before mlp adapter, v : ", v.shape)  
                            v = self.v_mlp_adapter(v)
                        elif self.perceptron_before_fusion:
                            v = self.v_perceptron_adapter(v)
                        #print("After extraction, v : ", v.shape)
                    elif self.video_feature_type == "cogenav":
                        
                        if grad:
                            v = self.feature_extractor.encoder.feature_extractor_video(v.contiguous())
                        else:
                            with torch.no_grad():
                                v = self.feature_extractor.encoder.feature_extractor_video(v.contiguous())
                    ...
                    if self.video_feature_type=="cogenav_after_transformer":
                        with torch.no_grad():
                            # ([2, 1, 51, 88, 88])
                            v = v.permute(0, 3, 4, 1, 2)
                            #(B, WH, WH, C, T) ([B, 88, 88, C=1, T=51])
                            v = self.feature_extractor(v, audio=None, use_upsampler=False).permute(0, 2, 1)
                            #print("after" , v.shape) # [B, 768, T]
        ...
        #if self.video_feature_type in  ["cogenav_pre", "avhubert_pre"]:
        if self.video_feature_type in  ["cogenav_pre", "avhubert_pre"]:  #$ ## only do this reshaping if we use fusion that use attn. 
            v = v.squeeze(1)  # remove the dimension corresponding to the channel (always 1)
