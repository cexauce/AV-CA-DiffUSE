...
from .ncsnpp_utils.utils import MultiModalAttention, mask_spectrogram
...
class NCSNpp_continueconcat_attn_masking_noising(nn.Module):
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        if not self.audio_only:
            ...
            elif self.fusion in ["concat_attn_masking_light_early_late", "early"]:
                self.attention_module = MultiModalAttention(x_feature_dim=256, v_feature_dim=self.new_embed_dim_video, num_heads=2)
                num_channels=3
                Concat_CrossAttn = functools.partial(
                    Concat_CrossAttnBlock_Light,
                    add_noise=False,
            ...
            if self.fusion in ["concat_attn_masking","concat_attn_masking_light", "concat_attn_masking_light_early_late", "concat_attn_noising","concat_attn","attn_gate","concat_attn_masking_learned","simple_attn"]:
        ...
        if not self.audio_only and self.fusion != "early":
    ...
    def audio_embedding(self, x):
        if self.video_feature_type=="cogenav":
            """AV-DiffSE : For the STFT, a Hann window of 510 length and a hop
            ...
            input sizes for batching."""
            #x_hat =  torch.zeros(B, 1, 256, 256, dtype = torch.complex64) # complex STFT
            x = torch.squeeze(x)
            print(x.shape)
            # take magnitude
            x = x.abs()**2
            ...
            # compute mel-spec
            """ 
            CoGenAV : extracting 80 bin log Mel spectrograms from audio sampled at
            
            sr = 16 kHz, with a stride of 10 ms and a window size of 25 ms.
            """
            sample_rate = 16e3
            mel = transforms.MelScale(n_mels = 80, sample_rate=sample_rate, n_stft=x.shape[-2])
            mel.to(x.device)

            x_mel = mel(x)
            x_mel = torch.log(x_mel + 1e-5)
            print(x_mel.shape)

            with torch.no_grad():
                fa_before_transformer = self.feature_extractor.encoder.feature_extractor_audio(x_mel.contiguous()) 
            print("audio feature : ", fa_before_transformer.shape)# [B, 768, T/4]
            return fa_before_transformer
    ...
    def forward(self, x, time_cond, v):
        ...
        if not self.audio_only and self.video_feature_type not in ["cogenav_pre", "avhubert_pre"]: #$ cogenav
            ...
            if self.vfeat_processing_order in ["cut_extract"]:
                ...
                elif self.video_feature_type=="cogenav_after_transformer":
                    with torch.no_grad():
                        # ([2, 1, 51, 88, 88])
